...
export class BillingEngineController {
  constructor(private service: BillingEngineService) {}

  @Get('rate-cards')
  @ApiOperation({ summary: 'List rate cards' })
  @ApiQuery({ name: 'client_id', required: false })
  getRateCards(@Req() req: TenantRequest, @Query('client_id') clientId?: string) {
    return this.service.getRateCards(req.tenant!.tenantId, clientId);
  }
  ...
  createExtraCharge(@Req() req: TenantRequest, @Body() dto: Record<string, unknown>) {
    return this.service.createExtraCharge(req.tenant!.tenantId, dto, dto.client_uuid as string);
  }
