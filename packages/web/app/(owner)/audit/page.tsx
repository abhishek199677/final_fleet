            ...
            <CardContent className="pt-6">
              <div className="grid gap-4 md:grid-cols-3 lg:grid-cols-6">
                <div>
                  ...
                  <NativeSelect className="mt-1 w-full" value={filters.user_id} onChange={(e) => setFilters({ ...filters, user_id: e.target.value })}>
                    <NativeSelectOption value="">All users</NativeSelectOption>
                    {users.map((u) => (
                      <NativeSelectOption key={String(u.id)} value={String(u.id)}>{String(u.name ?? u.email)}</NativeSelectOption>
                    ))}
                  </NativeSelect>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-700">Type</label>
                  <NativeSelect className="mt-1 w-full" value={filters.table} onChange={(e) => setFilters({ ...filters, table: e.target.value })}>
                    {TABLES.map((t) => (
                      <NativeSelectOption key={t} value={t}>{t === '' ? 'All types' : t.replace(/_/g, ' ')}</NativeSelectOption>
                    ))}
                  </NativeSelect>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-700">Machine</label>
                  <NativeSelect className="mt-1 w-full" value={filters.machine_id} onChange={(e) => setFilters({ ...filters, machine_id: e.target.value })}>
                    <NativeSelectOption value="">All machines</NativeSelectOption>
                    {machines.map((m) => (
                      <NativeSelectOption key={String(m.id)} value={String(m.id)}>{String(m.code)}</NativeSelectOption>
                    ))}
                  </NativeSelect>
                </div>
                <div>
            ...
            <CardContent className="pt-6">
              ...
              {!loading && rows.length > 0 && (
                <Marker variant="separator" className="mt-5">
                  <MarkerContent>
                    Showing {Math.min(50, rows.length)} of {rows.length} entries
                  </MarkerContent>
                </Marker>
              )}
            </CardContent>
